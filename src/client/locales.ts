...
export const en = {
  ...
  'row.searchedHint': 'No sign-in was found. These locations were probed; if your WorkBuddy stores its login elsewhere, point the plugin at it with the WORKBUDDY_AUTH_FILE environment variable.',
  ...
  'row.searchedEncryptedNotice': 'The credential file is there, but its token fields are encrypted and the key could not be read. Install the WorkBuddy desktop app, or set WORKBUDDY_APP_EXECUTABLE to its executable — signing in again on its own will not help.',
...
export const zh: Record<WorkBuddySettingsKey, string> = {
  ...
  'row.searchedHint': '未找到登录信息。以下是插件探测过的位置；如果你的 WorkBuddy 把登录信息存在别处，可以用环境变量 WORKBUDDY_AUTH_FILE 指定。',
  ...
  'row.sourceDsh': '插件副本',
  'row.searchedEncryptedNotice': '凭据文件在，但其中的 token 字段是加密的，插件拿不到密钥。请安装 WorkBuddy 桌面 App，或用环境变量 WORKBUDDY_APP_EXECUTABLE 指定它的可执行文件——单纯重新登录解决不了这个问题。',
