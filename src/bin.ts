...
async function doctor(jsonOutput: boolean): Promise<number> {
  ...
  const report = {
    ...
    hints: [
      ...
      ...accounts.length > 1 ? [] : ['Only one local WorkBuddy credential was found; account switching needs the app to have left backups of earlier sign-ins.'],
    ],
  }
  if (jsonOutput) {
    printJson(report)
...
async function status(jsonOutput: boolean): Promise<number> {
  ...
  if (authStatus.state !== 'signed-in') {
    if (jsonOutput) {
      printJson({ schemaVersion: JSON_SCHEMA_VERSION, package: 'dsh-connect-workbuddy', version: WORKBUDDY_CONNECT_VERSION, status: 'signed-out', hostBundle: hostState })
    } else {
  ...
  process.stdout.write([
    ...
    `Local accounts: ${accounts.length}${selected === undefined ? '' : ` (using ${selected.accountName})`}`,
