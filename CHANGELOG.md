...
- **保存国内版 / 国际版信息时，总有一个区域的配置被清掉**（用户实报：「我保存国际版信息和国内版信息，总会丢失一个」）。这是 2.0.13 那条「活引用被展开成函数」缺陷的**客户端侧同源路径**——服务端（`__save`）那次只修了一半，浏览器侧一直在按错误的方式读同一个值。
  ...
  - **修法**：两处都先 `unwrapVolatileDeep()` 再使用。该 helper 原本只存在于 Host 入口，浏览器侧取不到，因此**移入 `src/status-paths.ts`**——那个模块本来就是 host↔client 的无 node 依赖桥梁——并由 `src/index.ts` 继续对外导出，**公共 API 不变**。
  - **回归测试**（新增 `tests/volatile-scope.spec.ts`，7 例）：整个套件此前只用**普通对象**模拟 `getSnapshot().value`，而线上真实的形状是活引用——**受影响的路径从未被测过**。新用例按真实形状构造 scope，断言两个区域在每一种交替顺序下都存活、且文档里绝不出现引用函数。
  - **变异验证**：分别撤销两处修复——撤销 `fieldSnapshotOf` 的解包 → **5 例红**；撤销 `regionsMapOf` 的解包 → **2 例红**。两处修复各自都是必需的，没有一处是死代码。

  > 更正：2.0.13 的说明里我写过「对单 region 写入足够」。**那是错的**——每条写入路径都要靠这次读取来保住另一个区域，读错就会把它删掉。本次一并订正。

### Tests

- 332 → 339（新增 7 例：活引用形状下的双区域写入与开关回归）。
