...
2. **Credential paths are not verified per-item on real Windows hardware.** The candidate list is derived from platform conventions and pinned by unit tests (including both the unset and the blank-string fallback for `LOCALAPPDATA` / `APPDATA`), but **the development machine is a Mac, and a Windows-only write failure there is not merely unreproduced — it is impossible to reproduce**: on POSIX the same code never enters the retry branch. So if your credential lives outside the defaults, point `WORKBUDDY_AUTH_FILE` at it and please report what you see.
